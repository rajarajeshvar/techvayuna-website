...
export const InfiniteSpiral: React.FC<InfiniteSpiralProps> = ({
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (!scrollEnabled || !visibleRef.current || scrollDelta === 0 || hoveredRef.current) return;
    ...
    // Intercept mouse wheel scrolling when hovering over the spiral component
    const handleWheel = (e: WheelEvent) => {
      e.preventDefault();
      e.stopPropagation();
      targetProgressRef.current += clamp(
        e.deltaY * 0.003 * (Math.max(speed, 0.2) / 0.55),
        -1.5,
        1.5
      );
    };
  ...
  const rootStyle = {
    ...
    userSelect: (animationMode === 'drag' || animationMode === 'all' ? 'none' : 'auto') as React.CSSProperties['userSelect']
  };
    ...
    >
      <div className="infinite-spiral__stage" role="list" aria-label="Infinite spiral gallery">
        {normalizedItems.map((item, index) => {
          ...
          const itemKey = item.id ?? `${item.src}-${index}`;
          const commonProps = {
